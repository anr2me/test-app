...
let reducers = (state = initialState, action) => {
	...
	switch (action.type) { 
		case actions.ADD_ITEM:
			return {...state, itemList: state.itemList.concat({ id:uniqueId('item_'), sym:action.data, exrate:0.0, decimals:4 })};
		...
		case actions.FETCH_EXRATES_ERROR:
			return {...state, fetchPending:false, fetchError:action.error};
		default:      
			return state;  
	}
}
