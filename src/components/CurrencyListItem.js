								...
								<Grid.Column textAlign="left">
									<Grid>
										...
										<Grid.Column textAlign="right">
											<Label className="cur_amount">{Number(parseFloat(baseAmount * item.exrate).toFixed(item.decimals)).toLocaleString()}</Label>
										</Grid.Column>
									</Grid>
									<Label size="small">{item.sym} - {CurrencyNames[item.sym]}</Label>
									<Label size="small">1 {baseCurrency} = {item.sym} {Number(parseFloat(item.exrate).toFixed(item.decimals)).toLocaleString()}</Label>
								</Grid.Column>
								<Grid.Column width={4}>
